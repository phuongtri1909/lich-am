...
@section('title', 'Chỉnh sửa Meta Tags')
    ...
    <div class="form-card">
        <div class="form-header">
            <div class="form-title">
                ...
                <h5>Chỉnh sửa Meta Tags: {{ $locations[$metaTag->name] ?? $metaTag->name }}</h5>
            </div>
            <div class="category-meta">
                <div class="category-created">
                    <i class="fas fa-code"></i>
                    <span>Vị trí: {{ $locations[$metaTag->name] ?? $metaTag->name }}</span>
                </div>
                <div class="category-created">
                    <i class="fas fa-clock"></i>
                    <span>Ngày tạo: {{ $metaTag->created_at->format('d/m/Y H:i') }}</span>
                </div>
            </div>
        </div>
        <div class="form-body">
            <form action="{{ route('admin.meta-tags.update', $metaTag) }}" method="POST" class="category-form">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="custom-input {{ $errors->has('name') ? 'input-error' : '' }}" 
                        ...
                        @foreach($locations as $key => $label)
                            <option value="{{ $key }}" {{ old('name', $metaTag->name) == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div id="meta-tags-container">
                        @if($metaTag->meta_tags && count($metaTag->meta_tags) > 0)
                            @foreach($metaTag->meta_tags as $index => $tag)
                                <div class="meta-tag-item">
                                    <div class="row">
                                        <div class="col-md-5">
                                            <input type="text" class="custom-input" name="meta_tags[{{ $index }}][name]" 
                                                placeholder="Tên meta tag" value="{{ old('meta_tags.' . $index . '.name', $tag['name']) }}">
                                        </div>
                                        <div class="col-md-6">
                                            <input type="text" class="custom-input" name="meta_tags[{{ $index }}][content]" 
                                                placeholder="Nội dung meta tag" value="{{ old('meta_tags.' . $index . '.content', $tag['content']) }}">
                                        </div>
                                        <div class="col-md-1">
                                            <button type="button" class="btn btn-danger btn-sm remove-meta-tag" 
                                                style="{{ count($metaTag->meta_tags) > 1 ? 'display: block' : 'display: none' }}">
                                ...
                                <div class="row">
                                    <div class="col-md-5">
                                        <input type="text" class="custom-input" name="meta_tags[0][name]" 
                                            placeholder="Tên meta tag" value="{{ old('meta_tags.0.name') }}">
                                    </div>
                                    <div class="col-md-6">
                ...
                <div class="form-group">
                    <div class="checkbox-group">
                        <input type="checkbox" id="is_active" name="is_active" value="1" 
                            {{ old('is_active', $metaTag->is_active) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    let metaTagIndex = {{ $metaTag->meta_tags ? count($metaTag->meta_tags) : 1 }};
